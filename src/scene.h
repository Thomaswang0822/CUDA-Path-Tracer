...
struct DevScene {
    ...
    /**
     * Given a direction wi generated by BSDF sampling, return the
     * "what if" pdf value by light sampling.
     * 
     * @param pos: Shading point
     * @param wi: Sampled direction
     * @param radiance: Output parameter, radiance
     * @return float
     */
    __device__ float lightPdf(
        const glm::vec3 pos,
        const glm::vec3 wi,
        glm::vec3& radiance
    ) {
        float pLight;
        Ray lightRay = makeOffsetedRay(pos, wi);
        Intersection isecLight;
        this->intersect(lightRay, isecLight);
        isecLight.wo = -lightRay.direction;
        if (isecLight.primId == NullPrimitive) {
            // case 1: envMap background
            if (this->envMap != nullptr) {
                radiance = this->envMap->linearSample(Math::toPlane(lightRay.direction));
                pLight = this->environmentMapPdf(lightRay.direction);
            }
            // case 2: really didn't hit anything
            else {
            ...
            Material lightMat = this->getTexturedMaterialAndSurface(isecLight);
            // case 3: hit a mesh light
            if (lightMat.type == Material::Type::Light) {
                radiance = lightMat.baseColor;
                pLight = Math::pdfAreaToSolidAngle(
                    Math::luminance(radiance) * PiTwo * this->getPrimitiveArea(isecLight.primId) * this->sumLightPowerInv,
                    pos, isecLight.pos, isecLight.norm
                );
            }
            // case 4: hit some non-light mesh
            else {
                radiance = glm::vec3(0.f);
                pLight = 0.f;
            }
